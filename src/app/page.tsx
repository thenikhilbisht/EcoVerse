...
const features = [
  { icon: Cpu, title: "AI Carbon Tracking", desc: "Real-time AI analysis of your lifestyle to calculate and predict carbon emissions with personalized reduction strategies.", color: "#00ff88", href: "/#calculator" },
  { icon: Zap, title: "Gamified Challenges", desc: "Daily eco missions, streak rewards, XP points, and achievement badges to keep you motivated and engaged.", color: "#00d4ff", href: "/challenges" },
  { icon: Globe2, title: "Global Competition", desc: "Compete with eco-warriors worldwide. Weekly tournaments, team battles, and country leaderboards.", color: "#ffd700", href: "/leaderboard" },
  { icon: Users2, title: "Community Power", desc: "Share achievements, form teams, and collaborate on sustainability projects with people worldwide.", color: "#f97316", href: "/community" },
  { icon: TreePine, title: "Impact Dashboard", desc: "Beautiful real-time dashboard showing trees saved, CO₂ reduced, and your contribution to a healthier planet.", color: "#00ff88", href: "/dashboard" },
  { icon: Rocket, title: "Innovation Hub", desc: "Submit climate-tech startup ideas, participate in competitions, and connect with mentors and investors.", color: "#a78bfa", href: "/innovation" },
];
          ...
          <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-5">
            {features.map((f, i) => (
              <motion.div key={f.title} initial={{ opacity: 0, y: 30 }} whileInView={{ opacity: 1, y: 0 }}
                ...
                className="glass-card rounded-2xl p-6 group hover:scale-[1.02] transition-all duration-300 neon-border-hover border border-[rgba(0,255,136,0.08)]"
              >
                <div className="w-12 h-12 rounded-xl mb-4 flex items-center justify-center"
                  style={{ background: `${f.color}15`, border: `1px solid ${f.color}30` }} aria-hidden="true">
                ...
                <Link href={f.href} className="mt-4 inline-flex items-center gap-1 text-xs font-medium transition-colors" style={{ color: f.color }}>
                  Learn more about {f.title} <ArrowRight className="w-3.5 h-3.5 group-hover:translate-x-1 transition-transform" aria-hidden="true" />
                </Link>
              </motion.div>
            ))}
          </div>
            ...
            <div className="grid grid-cols-2 md:grid-cols-4 gap-6">
              {impactStats.map((stat, i) => (
                <motion.div key={stat.label} initial={{ opacity: 0, scale: 0.8 }} whileInView={{ opacity: 1, scale: 1 }}
                  ...
                  <stat.icon className="w-8 h-8 mx-auto mb-3" style={{ color: stat.color }} aria-hidden="true" />
          ...
          <div className="grid md:grid-cols-3 gap-6">
            {testimonials.map((t, i) => (
              <motion.div key={t.name} initial={{ opacity: 0, y: 30 }} whileInView={{ opacity: 1, y: 0 }}
                ...
                <div className="flex gap-1 mt-4" role="img" aria-label="Rated 5 out of 5 stars">
                  {[...Array(5)].map((_, s) => <span key={s} aria-hidden="true" className="text-[#ffd700] text-sm">★</span>)}
                </div>
              </motion.div>
            ))}
          </div>
        ...
        <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 relative text-center">
          <motion.div initial={{ opacity: 0, scale: 0.9 }} whileInView={{ opacity: 1, scale: 1 }} viewport={{ once: true }}>
            ...
            <div className="flex flex-col sm:flex-row gap-4 justify-center">
              <Link href="/challenges" className="btn-primary flex items-center gap-2 justify-center w-full sm:w-auto">
                <Zap className="w-4 h-4" aria-hidden="true" /> Join Free — Start Now
                <ArrowRight className="w-4 h-4" aria-hidden="true" />
              </Link>
              <Link href="/dashboard" className="btn-secondary flex items-center gap-2 justify-center w-full sm:w-auto">
                <Globe2 className="w-4 h-4" aria-hidden="true" /> View Dashboard
              </Link>
            </div>
